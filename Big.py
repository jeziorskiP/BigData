...
#2
print("--------P2.2--Complaint Type per Borough-----------")
start_time_process21 = time.time()
res = df.groupby('Borough')['Complaint Type']\
    .apply(pd.Series.value_counts)\
    .unstack().idxmax(axis=1)
print("--- %s seconds ---" % (time.time() - start_time_process21))
print(res)
print("----------!Complaint Type per Borough!------------------\n")
...
print("---------P3.2--Agency Name------------------ -------")
start_time_process33 = time.time()
onlyAgencyName = df.groupby([ 'Agency Name' ]).size().idxmax(0)

print("--- %s seconds ---" % (time.time() - start_time_process33))
